-- Exploratory queries: in-app notification data shape
-- Source: dim_notification_content, fct_notification_deliveries
-- Purpose: Inform LookML dashboard design for marketing
-- Author: d7admin
-- Date: 2026-04-02
-- Usage: Run each query (--qa through --qj) and export results as a.csv through j.csv
...
--qj  NULL notification_type audit
SELECT
    COUNT(*) AS unmatched_deliveries
    , COUNT(DISTINCT cms_entry_id) AS unmatched_distinct_entries
    , ROUND(100.0 * COUNT(*) / (SELECT COUNT(*) FROM soundstripe_prod.marketing.fct_notification_deliveries), 2) AS pct_of_all_deliveries
FROM soundstripe_prod.marketing.fct_notification_deliveries
