...
with time_limited_events as
    (
        select
            ...
            -- Schema verified via etl/tasks/2026-03-30-searched-songs-properties/schema_check/
            -- HAS_VOCALS: pre-3/25 values are boolean True/False; post-3/25 are enum All/Vocals/Instrumental. CASE NULLs out old booleans.
            ,CASE WHEN HAS_VOCALS IN ('All', 'Vocals', 'Instrumental') THEN HAS_VOCALS ELSE NULL END AS search_has_vocals
            ,TRY_CAST(RESULT_COUNT AS INTEGER) AS search_result_count  -- Searched Songs: number of results returned
            ,TRY_CAST(SUPE AS BOOLEAN) AS is_supe_search               -- Searched Songs: triggered from Supe AI chat sidebar (source is TEXT)
