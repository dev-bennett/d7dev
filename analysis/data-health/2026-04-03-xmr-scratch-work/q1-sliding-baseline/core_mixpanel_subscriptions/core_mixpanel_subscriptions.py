"""
XmR Process Behavior Chart -- Core Mixpanel Subscriptions (Daily, Sliding 90-Day Baseline)
...
df = pd.read_csv(DATA_DIR / "core_mixpanel_subscriptions.csv", parse_dates=["DATE"])
...
fig.text(
    0.06, 0.935,
    "Core Mixpanel Subscriptions (Daily)  \u2022  Sliding 90-Day Baseline  \u2022  Wheeler Signal Rules",
    fontsize=9, color=TEXT_SECONDARY, style="italic",
)

ax_x.set_ylabel("Core Mixpanel Subscriptions", fontsize=9.5, color=TEXT_SECONDARY, labelpad=8)
...
out_path = CHART_DIR / "core_mixpanel_subscriptions.png"
