"""
XmR Process Behavior Chart -- Paid Search Impressions (Daily, Sliding 90-Day Baseline)
...
df = pd.read_csv(DATA_DIR / "paid_search_impressions.csv", parse_dates=["DATE"])
...
fig.text(
    0.06, 0.935,
    "Paid Search Impressions (Daily)  \u2022  Sliding 90-Day Baseline  \u2022  Wheeler Signal Rules",
    fontsize=9, color=TEXT_SECONDARY, style="italic",
)

ax_x.set_ylabel("Paid Search Impressions", fontsize=9.5, color=TEXT_SECONDARY, labelpad=8)
...
out_path = CHART_DIR / "paid_search_impressions.png"
