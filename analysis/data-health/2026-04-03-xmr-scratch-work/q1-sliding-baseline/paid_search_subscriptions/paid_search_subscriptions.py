"""
XmR Process Behavior Chart -- Paid Search Subscriptions (Daily, Sliding 90-Day Baseline)
...
df = pd.read_csv(DATA_DIR / "paid_search_subscriptions.csv", parse_dates=["DATE"])
...
fig.text(
    0.06, 0.935,
    "Paid Search Subscriptions (Daily)  \u2022  Sliding 90-Day Baseline  \u2022  Wheeler Signal Rules",
    fontsize=9, color=TEXT_SECONDARY, style="italic",
)

ax_x.set_ylabel("Paid Search Subscriptions", fontsize=9.5, color=TEXT_SECONDARY, labelpad=8)
...
out_path = CHART_DIR / "paid_search_subscriptions.png"
