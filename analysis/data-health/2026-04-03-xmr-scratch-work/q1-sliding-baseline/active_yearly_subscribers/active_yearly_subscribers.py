"""
XmR Process Behavior Chart -- Active Yearly Subscribers (Daily, Sliding 90-Day Baseline)
...
df = pd.read_csv(DATA_DIR / "active_yearly_subscribers.csv", parse_dates=["DATE"])
...
fig.text(
    0.06, 0.935,
    "Active Yearly Subscribers (Daily)  \u2022  Sliding 90-Day Baseline  \u2022  Wheeler Signal Rules",
    fontsize=9, color=TEXT_SECONDARY, style="italic",
)

ax_x.set_ylabel("Active Yearly Subscribers", fontsize=9.5, color=TEXT_SECONDARY, labelpad=8)
...
out_path = CHART_DIR / "active_yearly_subscribers.png"
