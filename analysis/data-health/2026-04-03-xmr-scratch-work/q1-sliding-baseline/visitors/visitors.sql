-- Q1 Variant: Sliding 90-Day Baseline XmR -- VISITORS
-- Purpose: XmR chart with adaptive process limits using a trailing 90-day window
-- Author: d7admin
-- Date: 2026-04-06
-- Source: soundstripe_prod.core.dim_daily_kpis
-- Methodology: Wheeler XmR with rolling baseline -- limits recalculate daily
...
--
-- DIFFERENCE FROM ORIGINAL Q1:
-- Original uses hardcoded baseline_start/baseline_end dates.
-- This version derives the baseline window from data_start - baseline_days,
-- so the trailing window is always fully populated for the analysis period.

-- ============================================================================
-- Q1-S: Single-Metric XmR -- VISITORS (Sliding 90-Day Baseline)
-- ============================================================================

WITH config AS (
    SELECT
        90 AS baseline_days                                -- trailing window size
        ,'2026-01-01'::DATE AS data_start                  -- analysis period starts here
        ,DATEADD('day', -90, data_start) AS baseline_start  -- derived: load data from here
)
...
-- Sliding baseline: x_bar and mr_bar over the trailing 90 days for each row
,sliding_baseline AS (
    SELECT
        w.date
        ,w.value
...
,xmr_chart AS (
    SELECT
        ...
        ,CASE
            WHEN sb.value > LAG(sb.value, 1) OVER (ORDER BY sb.date) THEN 1
            WHEN sb.value < LAG(sb.value, 1) OVER (ORDER BY sb.date) THEN -1
            ELSE 0
         END AS direction
...
,with_runs AS (
    SELECT
        ...
        ,ROW_NUMBER() OVER (ORDER BY x.date)
         - ROW_NUMBER() OVER (PARTITION BY x.direction ORDER BY x.date) AS trend_group
    FROM xmr_chart x
)
